  ...
  useFrame((threeState, delta) => {
    ...
    switch (s.mode) {
      ...
      case STATE.APPROACHING:
        ...
        s.speed = THREE.MathUtils.lerp(s.speed, 2, delta); // 接近時減速
      ...
      case STATE.OBSERVING:
        ...
        // 微微漂浮
        s.targetPos.y += Math.sin(time) * 0.005;
        
        // 觀察 3-6 秒後離開
        if (s.timer > 3 + Math.random() * 3) {
    ...
    camera.position.lerp(s.targetPos, moveStep * 0.5);
    ...
    // 添加一點無人機般的晃動 (Noise)
    camera.position.y += Math.sin(time * 2 + s.noiseOffset) * 0.002;
    camera.rotation.z = Math.sin(time * 0.5) * 0.02; // 輕微傾斜
  });
